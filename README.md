...
- [x] **URLSession / Alamofire DoH gateway (iOS 15)** — Forum API, topic lists, password login, and URLSession images can use a custom HTTPS DoH URL through an app-local loopback gateway. Built-in resolvers plus custom. Switching probes off the main thread (~4s cap) and reverts or disables on failure so launch never black-screens. This is the API path only — not a full-browser DoH proxy
