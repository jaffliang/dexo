...
pub extern "C" fn dexo_doh_gateway_start(doh_url: *const c_char, preferred_port: u16) -> i32 {
    ...
    let started = runtime().block_on(async {
        ...
        let resolver = DohResolver::new(url, tls.clone()).map_err(|_| -1i32)?;
        let listener = TcpListener::bind(&bind_addr).await.map_err(|_| -2i32)?;
        listener
            .local_addr()
            .map_err(|_| -2i32)
            .map(|addr| (addr.port(), listener, resolver, tls))
    });
